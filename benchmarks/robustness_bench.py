...
from _example_utils import (  # noqa: E402
    axis_angle_R, rot_angle_deg, chamfer_mm, sim3_matrix, make_object_splat,
)
...
def _clone_with(g, means, idx=None):
    ...
    return Gaussians(means=means,
                     quats=sel(g.quats).clone(), scales=sel(g.scales).clone(),
                     opacities=sel(g.opacities).clone(), log_scales=g.log_scales)
...
def perturb_outliers(B, frac, seed):
    ...
    return Gaussians(means=means, quats=q.clone(), scales=s.clone(), opacities=o.clone(),
                     log_scales=B.log_scales)
...
def make_symmetric_splat(n, seed):
    ...
    r = 0.10
    u = torch.rand(n, generator=g); v = torch.rand(n, generator=g)
    phi = 2 * math.pi * u; costh = 2 * v - 1
    ...
    pts = (sph * r * rad).to(device=DEVICE, dtype=DTYPE) + torch.tensor([0.4, -0.2, 0.3], device=DEVICE, dtype=DTYPE)
    m = pts.shape[0]
    return Gaussians(means=pts,
                     quats=torch.tensor([1.0, 0, 0, 0], device=DEVICE, dtype=DTYPE).repeat(m, 1),
                     scales=torch.full((m, 3), 0.004, device=DEVICE, dtype=DTYPE),
                     opacities=torch.ones(m, device=DEVICE, dtype=DTYPE), log_scales=False)
...
def run_condition(name, levels, level_label, build_B, transform="se3", symmetric=False):
    print("=" * 96)
    print(f"{name}   (gate: {'Chamfer<%.1fmm (rotation ambiguous)' % CHAMFER_GATE_MM if symmetric else 'rot<%.1f deg' % ROT_GATE})")
    print(f"{'level':>10} {'seed':>5} | {'rot_err deg':>11} {'chamfer_mm':>11} {'sec':>7}  ok")
    print("-" * 96)
    rows = []
    for lvl in levels:
        for seed in SEEDS:
            A = make_object_splat(N_POINTS, seed=seed, device=DEVICE, dtype=DTYPE) if not symmetric \
            ...
            print(f"{level_label(lvl):>10} {seed:>5} | {rot_err:>11.4f} {cham:>11.4f} {dt:>7.1f}  {'Y' if ok else '.'}")
    n_ok = sum(r[4] for r in rows)
    print("-" * 96)
    print(f"  {name} SUMMARY: {n_ok}/{len(rows)} within gate "
...
def main():
    ...
    summ = []
    summ.append(run_condition(
        "NOISE (sensor jitter)", [0.005, 0.01, 0.02], lambda x: f"{x * 100:.1f}%R",
        lambda B, lvl, seed: perturb_noise(B, lvl * OBJ_RADIUS, seed)))
    summ.append(run_condition(
        "PARTIAL (occlusion)", [0.8, 0.6, 0.4], lambda x: f"keep{int(x * 100)}%",
        lambda B, lvl, seed: perturb_partial(B, lvl, seed)))
    summ.append(run_condition(
        "OUTLIERS (clutter)", [0.1, 0.25, 0.5], lambda x: f"+{int(x * 100)}%",
        lambda B, lvl, seed: perturb_outliers(B, lvl, seed)))
    summ.append(run_condition(
        "SYMMETRIC (sphere, no lobe)", [5.0, 30.0, 90.0], lambda x: f"{x:.0f}deg",
        None, symmetric=True))

    print("=" * 96)
    tot_ok = sum(s[1] for s in summ); tot = sum(s[2] for s in summ)
    for name, n_ok, n, _ in summ:
        print(f"  {name:30s}: {n_ok}/{n} ({100.0 * n_ok / n:.0f}%)")
    print(f"  OVERALL ROBUSTNESS: {tot_ok}/{tot} ({100.0 * tot_ok / tot:.0f}%)   "
