...
from splatreg.io import _read_ply_vertex
...
def read_gt_log(path: str) -> dict[tuple[int, int], np.ndarray]:
    ...
    ``gt.log`` stores four-line blocks prefixed by ``target source
    number_of_fragments``.  The matrix maps the second fragment into the first
    fragment's frame.  This direction matches the official 3DMatch evaluator
    and GeoTransformer metadata reader.
    """
    ...
    while cursor < len(lines):
        ...
        target, source = int(header[0]), int(header[1])
...
def gather_pairs(
    ...
    # Each candidate is (scene, source index, target index, source path,
    # target path, source->target transform or None).  The explicit paths
    # prevent a silent convention reversal in the flat official archive.
    candidates = []
    if not scenes and list(root.glob("cloud_bin_*.ply")):
        ...
        for (target, source), transform in transforms.items():
            if target not in by_index or source not in by_index:
                continue
            candidates.append(
                (root, source, target, by_index[source], by_index[target], transform)
            )
    for sc in scenes:
        ...
        for i in range(n):
            for j in range(i + 1, min(i + 4, n)):
                candidates.append((sc, i, j, frags[i], frags[j], None))
    rng.shuffle(candidates)
    pairs = []
    for sc, source, target, source_path, target_path, logged_transform in candidates:
        if len(pairs) >= n_pairs:
            break
        try:
            pi = read_points(str(source_path))
            pj = read_points(str(target_path))
            if logged_transform is not None:
                T_gt = logged_transform
            else:
                posei = read_pose(str(sc / "poses" / f"{source_path.stem}.txt"))
                posej = read_pose(str(sc / "poses" / f"{target_path.stem}.txt"))
...
def wilson_interval(successes: int, total: int, z: float = 1.959963984540054) -> tuple[float, float]:
    """Two-sided Wilson score interval for a binomial recall proportion."""
    if total <= 0:
        return 0.0, 1.0
    p = successes / total
    denom = 1.0 + z * z / total
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="skip this many pairs in the deterministic shuffled list (for frozen dev/test slices)",
    )
    ...
    ap.add_argument(
        "--oracle-diagnostic",
        action="store_true",
        help="start only the local LM at ground truth to separate refinement from initialization failure",
    )
    ap.add_argument("--output-json", default=None, help="optional machine-readable per-pair artifact")
    ap.add_argument(
        "--init",
        default="robust",
    ...
    if args.pair_offset < 0:
        ap.error("--pair-offset must be nonnegative")
    pairs = gather_pairs(
        args.data_root,
        args.voxel,
        args.voxel * 2.0,
        args.min_overlap,
        args.n_pairs + args.pair_offset,
        args.seed,
        args.gt_log,
    )
    pairs = pairs[args.pair_offset :]
    if not pairs:
        raise RuntimeError("no pairs remain after overlap filtering and pair offset")
    print(
        f"  -> {len(pairs)} pairs across "
        f"{len({p['scene'] for p in pairs})} scenes (median overlap "
    ...
    for k, p in enumerate(pairs):
        ...
        if args.no_basin_sweep:
            T_est, sr_info = feature_align(gs_tgt, gs_src, transform="se3", basin_sweep=False)
        else:
            res = register(
                gs_tgt,
                gs_src,
        ...
        record = {
            ...
            "splatreg": {
                "rre_deg": sr["rre"][-1],
                "rte_m": sr["rte"][-1],
                "corr_rmse_m": sr["rmse"][-1],
                "time_ms": sr["ms"][-1],
                "ambiguous": bool(sr_info.get("ambiguous", False)),
                "confidence": float(sr_info.get("confidence", 0.0)),
            },
        }
        ...
        if args.oracle_diagnostic:
            if device.startswith("cuda"):
                torch.cuda.synchronize()
            t0 = time.perf_counter()
            oracle_result = register(
                gs_tgt,
                gs_src,
                init=torch.as_tensor(T_gt, device=device, dtype=torch.float32),
                transform="se3",
                quality=args.quality,
                max_iters=args.max_iters,
            )
            if device.startswith("cuda"):
                torch.cuda.synchronize()
            oracle["ms"].append((time.perf_counter() - t0) * 1000.0)
            T_oracle = oracle_result.T.detach().cpu().numpy().astype(np.float64)
            oracle["rre"].append(rot_err_deg(T_gt[:3, :3], T_oracle[:3, :3]))
            oracle["rte"].append(float(np.linalg.norm(T_gt[:3, 3] - T_oracle[:3, 3])))
            oracle["rmse"].append(correspondence_rmse(src, tgt, T_gt, T_oracle, args.corr_band))
            record["oracle_refine"] = {
                "rre_deg": oracle["rre"][-1],
                "rte_m": oracle["rte"][-1],
                "corr_rmse_m": oracle["rmse"][-1],
                "time_ms": oracle["ms"][-1],
            }
    ...
    def report(name, d):
        ...
        successes = int(np.count_nonzero(succ))
        ci_lo, ci_hi = wilson_interval(successes, len(rmse))
        print(f"\n=== {name} (n={len(d['rre'])}) ===")
        print(
            f"  RR (RMSE<{args.rmse_thresh} m)   : {rr * 100:.1f}% "
            f"(95% Wilson {ci_lo * 100:.1f}-{ci_hi * 100:.1f}%)"
        )
        ...
        return {
            ...
            "total": len(rmse),
            "recall": rr,
            "recall_wilson_95": [ci_lo, ci_hi],
            "median_rre_deg_all": float(np.median(d["rre"])),
            "median_rte_m_all": float(np.median(d["rte"])),
            "median_time_ms": float(np.median(d["ms"])),
        }

    print("\n" + "=" * 60)
    summaries = {"splatreg": report(f"splatreg  (init={args.init!r})", sr)}
    if has_o3d:
        summaries["open3d"] = report("Open3D  FPFH+RANSAC", o3)
    if args.oracle_diagnostic:
        summaries["oracle_refine"] = report("Oracle-initialized local refinement (diagnostic)", oracle)
    print("=" * 60)
    ...
    if args.output_json:
        ...
        artifact = {
            "schema_version": "1.0",
            "protocol": "bounded-3dmatch-diagnostic",
            "transform_convention": "gt.log matrix maps second/header source into first/header target",
            "config": {
                "data_root": str(Path(args.data_root).resolve()),
                "gt_log": str(Path(args.gt_log).resolve()) if args.gt_log else None,
                "seed": args.seed,
                ...
                "quality": args.quality,
                "max_iters": args.max_iters,
                "basin_sweep": not args.no_basin_sweep,
                "min_overlap": args.min_overlap,
                "corr_band_m": args.corr_band,
                "rmse_threshold_m": args.rmse_thresh,
                "device": device,
            },
            "summaries": summaries,
            "pairs": pair_records,
