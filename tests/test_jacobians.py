...
def _make_splat(n: int, seed: int) -> Gaussians:
    ...
    return Gaussians(means=means.to(DEV), quats=q.to(DEV), scales=scales.to(DEV),
                     opacities=torch.ones(n, dtype=DTYPE, device=DEV), log_scales=False)
...
def _check(name, J_an, J_num, atol):
    ...
    ok = err < atol
    print(f"  [{'OK  ' if ok else 'FAIL'}] {name:24s} max|Δ|={err:.2e}  (atol {atol:.0e}, {tuple(J_an.shape)})")
    return ok
...
def audit_residual_jacobians():
    ...
    for tname, T in [("near-identity", _rand_T(0.05)), ("large-rotation", _rand_T(0.9, 11))]:
        ...
        results.append(_check("ICP point-to-point",
        ...
        results.append(_check("ICP point-to-plane",
                              icp_pl.jacobian(T, target, source),
                              _num_jac(lambda Tp, s=src2, qq=q2, nn=n2: ((s @ Tp[:3, :3].T + Tp[:3, 3] - qq) * nn).sum(-1), T),
                              1e-5))
        sdf = SDF(sigma=0.02, n_points=0)
        src_s = sdf._source_points(source)
        results.append(_check("SDF",
                              sdf.jacobian(T, target, source),
                              _num_jac(lambda Tp, s=src_s, sd=sdf, tg=target: sd._sdf(tg, s @ Tp[:3, :3].T + Tp[:3, 3])[0] * sd.weight, T),
                              1e-3))
    return results
